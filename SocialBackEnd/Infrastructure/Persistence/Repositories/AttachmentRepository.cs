...
public sealed class AttachmentRepository : RepositoryBase<Attachments>, IAttachmentRepository
{
    ...
    {
        var attachments = await DbContext.Set<Attachments>().Where(a => a.PostId == postId).ToListAsync();
        if (attachments == null || attachments.Count == 0)
        {
            return false; // Không tìm thấy tệp đính kèm nào cho bài viết này
        }

        DbContext.Set<Attachments>().RemoveRange(attachments);
        await DbContext.SaveChangesAsync();
        return true;
    }
    ...
    {
        return await DbContext.Set<Attachments>().Where(a => a.PostId == postId).ToListAsync(cancellationToken);
    }
}
