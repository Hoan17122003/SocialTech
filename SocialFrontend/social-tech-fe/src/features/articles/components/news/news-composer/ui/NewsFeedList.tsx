...
export function NewsFeedList({ articles, cardProps }: Props) {
    return <div className="mx-auto mt-8 flex max-w-3xl flex-col gap-6">{articles.map((article, index) => <ArticleCard key={article.id} article={article} index={index} {...cardProps} />)}</div>;
}
