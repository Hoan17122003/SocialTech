...
export function ProfilePanel({ userId }: { userId: number }) {
    ...
    useEffect(() => {
        ...
        async function loadProfile() {
            try {
                ...
                if (isMounted) {
                    setError(cause instanceof ApiError ? cause.message : 'Khong the tai profile.');
                }
            }
        }
    ...
    async function handleUpdate(event: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            setError(cause instanceof ApiError ? cause.message : 'Khong the cap nhat profile.');
        } finally {
                            ...
                            {profile.recentPosts.length ? (
                                profile.recentPosts.map((post) => (
                                    ...
                                    >
                                        ...
                                        <p className="mt-2 text-sm leading-7 text-[var(--muted)]">{post.body}</p>
                                    </div>
                                ))
                            ) : (
