                ...
                <div className="grid gap-6 mt-4">
                    ...
                    <div className="rounded-[2rem] border border-[var(--line)] bg-[var(--surface)] p-6 shadow-[var(--shadow)] relative overflow-hidden flex flex-col sm:flex-row items-center gap-6">
                        ...
                        {profile.isPermissionEdit && (
                            ...
                            >
                                <svg className="h-5 w-5 animate-spin-hover" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                    ...
                    <div className="grid gap-4 grid-cols-3">
                        ...
                        <StatCard 
                            label="Quyền soạn" 
                            value={profile.isPermissionEdit ? 'Có' : 'Không'} 
                            icon={
                    ...
                    <div className="rounded-[2rem] border border-[var(--line)] bg-[var(--surface)] p-6 shadow-[var(--shadow)]">
                        ...
                        <div className="grid gap-4">
                            {profile.recentPosts.length ? (
                                ...
                                <p className="text-sm text-[var(--muted)] text-center py-6">Chưa có bài viết nào được đăng tải.</p>
                            )}
                        </div>
                    </div>
                </div>
                    ...
                    >
                        ...
                        <div className="flex items-start justify-between gap-4 border-b border-[var(--line)] pb-4 mb-6 relative z-10">
                            <div>
                                ...
                                <h2 className="text-2xl font-extrabold text-[var(--foreground)] tracking-tight">Cấu hình tài khoản</h2>
                            </div>
                            <button
                                type="button"
                        ...
                        <form onSubmit={handleUpdate} className="space-y-5 relative z-10">
                            {settingsTab === 'profile' ? (
                                <div className="space-y-4 animate-fade-in">
                                    ...
                                    {/* Bio */}
                                    <div className="space-y-1.5">
                                        <label className="text-xs font-semibold text-[var(--foreground)] opacity-85">Tiểu sử (Bio)</label>
                                        <Textarea
                                            value={bio}
                                    ...
                                    {/* File Avatar */}
                                    <div className="space-y-1.5">
                                        <label className="text-xs font-semibold text-[var(--foreground)] opacity-85">Ảnh đại diện mới</label>
                                        <Input
                                            type="file"
                                            accept="image/*"
                                ...
                                <div className="space-y-4 animate-fade-in">
                                    ...
                                    <div className="space-y-1.5">
                                        <label className="text-xs font-semibold text-[var(--foreground)] opacity-85">Mật khẩu mới (Bỏ trống nếu không đổi)</label>
                                        <div className="relative">
                                            <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-4 text-[var(--muted)]">
                                                <svg className="h-4.5 w-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                                                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                                    ...
                                    <div className="flex items-center justify-between p-4 rounded-2xl border border-[var(--line)] bg-[var(--background-soft)]">
                                        <div className="space-y-0.5">
                                            <p className="text-sm font-bold text-[var(--foreground)]">Chế độ riêng tư</p>
                                            <p className="text-xs text-[var(--muted)]">Chỉ cho phép follower xem nội dung bài viết.</p>
                                        </div>
                                        <input
                                            type="checkbox"
                                            checked={isPrivate}
                                    ...
                                    <div className="flex items-center justify-between p-4 rounded-2xl border border-[var(--line)] bg-[var(--background-soft)]">
                                        <div className="space-y-0.5">
                                            <p className="text-sm font-bold text-[var(--foreground)]">Chế độ giao diện</p>
                                            <p className="text-xs text-[var(--muted)]">Chọn giao diện Tối (Dark) hoặc Sáng (Light).</p>
                                        </div>
                                        <button
                                            type="button"
                                            onClick={toggleTheme}
    ...
    return (
        <div className="glow-card rounded-[1.75rem] border border-[var(--line)] bg-[var(--surface)] p-5 shadow-sm relative overflow-hidden flex flex-col justify-between min-h-[110px]">
            <div className="flex items-center justify-between gap-2">
                ...
                {icon && <div className="p-1.5 rounded-lg bg-[var(--background-soft)] border border-[var(--line)]">{icon}</div>}
            </div>
            <p className="mt-3 text-2xl font-extrabold text-[var(--foreground)] tracking-tight">
                {value}
            </p>
        </div>
    );
